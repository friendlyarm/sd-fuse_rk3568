...
BOARD=rk3568/images-for-eflasher

TARGET_OS=$(echo ${1,,}|sed 's/\///g')
ROMFILE=`./tools/get_pkg_filename.sh ${TARGET_OS}`
if [ -z ${ROMFILE} ]; then
	echo "Usage: $0 <buildroot|friendlycore-focal-arm64|openmediavault-arm64|debian-buster-desktop-arm64|debian-bullseye-desktop-arm64|friendlywrt23|friendlywrt23-docker|friendlywrt22|friendlywrt22-docker|friendlywrt21|friendlywrt21-docker|eflasher>"
	exit 1
fi
